...
{
  const j = S.justName;
  eq("기관이 앞에", j("국토연구원 김중은"), "김중은");
  eq("기관이 뒤에", j("박신원 LH토지주택연구원"), "박신원");
  eq("괄호 소속", j("박진우 (수원시정연구원)"), "박진우");
  eq("소속과 직함까지", j("이석준 이천시청 군협력담당관"), "이석준");
  eq("이름만이면 그대로", j("강한구"), "강한구");
  eq("두 글자도", j("남편"), "남편");
  eq("한 글자도", j("김"), "김");
  eq("이름이 「원」으로 끝나도 안 지운다", j("이서원 서울연구원"), "이서원");
  eq("기관뿐이면 앞 낱말", j("수원시정연구원 담당"), "수원시정연구원");
  eq("짧은 기관이 앞에 — 이천시", j("이천시 이석준"), "이석준");
  eq("짧은 기관이 앞에 — 경기도", j("경기도 김철수"), "김철수");
  eq("짧은 기관이 앞에 — 서울대", j("서울대 이영희"), "이영희");
  eq("짧은 기관이 앞에 — 국토부", j("국토부 박민수"), "박민수");
  eq("이름이 「대」로 끝나도 혼자면 그대로", j("한서대"), "한서대");
}

console.log("\n── 연구 주제 낱말 ──");
{
  const rows = [
    { title: "이천 방문", body: "청미천에서 드론을 띄웠다. 빈집 정비도 이야기함." },
    { title: "회의", body: "K-컬처 거점과 방위산업 클러스터" },
    { title: "메모", body: "드론 교육원 답사" },
    { title: "영문", body: "CHAIRMAN 이 왔다" },
  ];
  const w = S.themeWords(rows, 20);
  const n = (k) => (w.find((x) => x.word === k) || {}).n;
  eq("드론이 두 번", n("드론"), 2);
  eq("빈집이 한 번", n("빈집"), 1);
  eq("K-컬처를 K컬처로", n("K컬처"), 1);
  eq("방위산업을 방산으로", n("방산"), 1);
  eq("CHAIRMAN 안에서 AI 를 찾지 않는다", n("데이터·AI"), undefined);
  eq("잦은 것부터", w[0].word, "드론");
