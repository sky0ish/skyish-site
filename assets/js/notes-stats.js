 ...
 */
/* 「국토연구원 김중은」 · 「이석준 이천시청 군협력담당관」 처럼
   기관·직함이 섞인 이름표에서 사람 이름만 꺼냅니다.
   낱말이 한글 2~4자이고 기관을 뜻하는 말이 섞이지 않았으면 이름으로 봅니다. */
const ORG_WORD =
  /(연구원|연구소|시청|군청|도청|구청|공사|공단|대학교|대학원|대학|재단|협회|학회|센터|본부|본사|지사|사무소|위원회|주식회사|법인)/;
/* 낱말 하나가 통째로 직함이면 이름이 아닙니다 */
const JOB_WORD = new Set([
  "담당", "담당관", "주무관", "사무관", "서기관", "팀장", "과장", "부장", "차장",
  "실장", "국장", "본부장", "원장", "소장", "센터장", "지사장", "교수", "강사",
  "박사", "위원", "위원장", "연구원", "연구위원", "책임", "선임", "대표", "이사",
  "사장", "부사장", "상무", "전무", "감사", "회장", "총장", "학장", "대리", "주임",
  "기사", "선생", "선생님", "님", "의원", "시장", "군수", "구청장", "도지사",
]);
export function justName(label) {
  const bare = String(label || "").replace(/\s*[(（].*$/, "").trim();
  const parts = bare.split(/\s+/).filter(Boolean);
  if (parts.length <= 1) return bare;
  const cand = parts.filter((w) =>
    /^[가-힣]{2,4}$/.test(w) && !ORG_WORD.test(w) && !JOB_WORD.has(w));
  /* 「이천시 이석준」 처럼 짧은 기관이 앞에 올 때가 있습니다.
     다른 후보가 있을 때만 밀어냅니다 — 그래야 「이서원」 같은 이름이 안 다칩니다. */
  const person = cand.filter((w) => !/(시|도|군|구|부|처|청|대)$/.test(w));
  return person[0] || cand[0] || parts[0];
}

/* ── 연구 주제 키워드 ──────────────────────────────────────
   글 어디에 적혀 있든(제목·본문·행사명·말머리·장소) 찾아냅니다.
   왼쪽이 그림에 뜨는 이름, 오른쪽이 그 이름으로 볼 말들입니다. */
export const THEMES = [
  ["K컬처",     ["K컬처", "K-컬처", "K컬쳐", "K-컬쳐", "케이컬처", "한류", "K팝", "K-POP", "K콘텐츠"]],
  ["드론",      ["드론", "UAM", "무인기", "무인항공", "드론교육", "드론훈련"]],
  ["빈집",      ["빈집", "빈 집", "공가", "폐가", "유휴건축"]],
  ["방산",      ["방산", "방위산업", "국방", "군사시설", "군사보호", "군협력", "무기체계", "군부대"]],
  ["건축",      ["건축", "설계공모", "리모델링", "건축물", "건축사"]],
  ["균형발전",  ["균형발전", "지역균형", "낙후지역", "지역발전", "소외지역", "지역격차"]],
  ["도시재생",  ["도시재생", "재생사업", "뉴딜", "쇠퇴지역"]],
  ["반도체",    ["반도체", "파운드리", "시스템반도체"]],
  ["스마트시티", ["스마트시티", "스마트도시", "디지털트윈"]],
  ["주거",      ["주거", "주택", "아파트", "임대주택", "공공주택", "정비사업", "재개발", "재건축"]],
  ["문화예술",  ["문화예술", "미술관", "박물관", "공연장", "문화시설", "예술인"]],
  ["관광",      ["관광", "체류형", "관광객"]],
  ["교통",      ["교통", "철도", "GTX", "광역버스", "역세권"]],
  ["산업단지",  ["산업단지", "산단", "공업지역", "물류단지"]],
  ["기후환경",  ["탄소중립", "기후", "온실가스", "침수", "홍수", "그린벨트", "개발제한구역"]],
  ["공원녹지",  ["공원", "녹지", "정원", "하천"]],
  ["인구감소",  ["인구감소", "지방소멸", "고령화", "저출생", "인구구조"]],
  ["청년",      ["청년", "대학생", "청년정책"]],
  ["데이터·AI", ["인공지능", "AI", "빅데이터", "데이터센터"]],
  ["안전방재",  ["재난", "방재", "안전관리", "침수피해"]],
  ["접경지역",  ["접경", "DMZ", "비무장지대", "남북", "통일"]],
  ["도시계획",  ["도시계획", "지구단위", "용도지역", "공간계획", "기본계획"]],
];

/* 한글 낱말은 그대로 찾고, 영문·숫자 낱말은 앞뒤가 글자가 아닐 때만 셉니다.
   그래야 「AI」 가 「CHAIR」 안에서 걸리지 않습니다. */
function hasWord(hay, w) {
  if (/[가-힣]/.test(w)) return hay.indexOf(w) >= 0;
  const H = hay.toUpperCase(), W = w.toUpperCase();
  for (let i = H.indexOf(W); i >= 0; i = H.indexOf(W, i + 1)) {
    const a = H.charAt(i - 1), b = H.charAt(i + W.length);
...
/** 글 한 편에 든 주제 이름들 */
export function themesOf(row) {
  const hay = [row.title, row.body, row.event, row.tag, row.place]
    .filter(Boolean).join(" ");
  return THEMES.filter(([, keys]) => keys.some((w) => hasWord(hay, w)))
               .map(([label]) => label);
}

/** 자주 나온 주제부터 n 개 */
export function themeWords(rows, n = 14) {
  const c = new Map();
  rows.forEach((r) => themesOf(r).forEach((t) => c.set(t, (c.get(t) || 0) + 1)));
  return [...c.entries()].map(([word, k]) => ({ word, n: k }))
